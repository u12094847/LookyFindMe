...
if (isset($_POST['method'])) {
    ...
    if (strcasecmp($method, 'login') == 0 and isset($_POST['username']) and isset($_POST['password'])) {
    ...
    } else if (strcasecmp($method, 'getPending') == 0 and isset($_POST['username'])) {
        $username = $_POST['username'];
        ...
        if (!empty($username)) {
            ...
            if (mysqli_num_rows($sql) > 0) {
                if ($result = mysqli_fetch_row($sql)) {
                    $sql = mysqli_query($db, "SELECT friend_one FROM friend WHERE user_id='$result[0]' and friend_status = 2");
                    if (mysqli_num_rows($sql) > 0) {
                        $jsonData = array();
    ...
    } else if (strcasecmp($method, 'getRequests') == 0 and isset($_POST['username'])) {
        $username = $_POST['username'];
        // Input validations
        if (!empty($username)) {
            //This SQL statement gets all the usernames for a person/user
            ...
            if (mysqli_num_rows($sql) > 0) {
                if ($result = mysqli_fetch_row($sql)) {
                    $sql = mysqli_query($db, "SELECT friend_one FROM friend WHERE user_id='$result[0]' and friend_status = 1");
                    if (mysqli_num_rows($sql) > 0) {
                        $jsonData = array();
