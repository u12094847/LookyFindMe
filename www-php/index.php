...
if (isset($_POST['method'])) {
    ...
    if (strcasecmp($method, 'login') == 0 and isset($_POST['username']) and isset($_POST['password'])) {
        ...
        // Input validations
        if (!empty($username)) {
                $sql = mysqli_query($db, "SELECT DISTINCT user.user_username FROM user INNER JOIN friend ON (user.user_id = friend.friend_one) OR (user.user_id = friend.friend_two) WHERE friend.friend_status = 3");
            if (mysqli_num_rows($sql) > 0) {
                $result = mysqli_fetch_array($sql, MYSQLI_ASSOC);
                $response['data'] = json($result);
                $response['success'] = true;
